...
void setupCLS(void)
{
	...
	SpiChnPutS(SPI_CHANNEL2,enable_display,strlen(enable_display));
	SpiChnPutS(SPI_CHANNEL2,set_cursor,strlen(set_cursor));
	SpiChnPutS(SPI_CHANNEL2,home_cursor,strlen(home_cursor));
	SpiChnPutS(SPI_CHANNEL2,wrap_line,strlen(wrap_line));
}
void vSetupTasks(void *pvParameters)
{
...
void vTaskFunction( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		{
			SpiChnPutS(SPI_CHANNEL2,home_cursor,strlen(home_cursor));
			SpiChnPutS(SPI_CHANNEL2, pcTaskName,strlen(pcTaskName) -1);
